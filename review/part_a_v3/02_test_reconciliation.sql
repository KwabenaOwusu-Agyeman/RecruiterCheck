-- ============================================================================
-- TEST-PROJECT-ONLY reconciliation migration. Target:
-- myrecruitercheck-scoring-test. NOT applied yet. Idempotent: safe to run
-- more than once against the same database state.
-- ============================================================================

-- Whole-migration atomicity: if any statement below fails (including the
-- fail-closed purchase-row precondition in Step 8), everything in this file
-- rolls back -- no partial schema change is ever left behind.
begin;
...
declare
  ...
  v_paid_without_batch integer;
begin
  select count(*) into v_null_credit_source from public.keyword_scan_reservations where credit_source is null;
  select count(*) into v_paid_without_batch from public.keyword_scan_reservations where credit_source = 'paid' and batch_id is null;
  raise notice 'Reconciliation audit: % null credit_source, % paid-without-batch', v_null_credit_source, v_paid_without_batch;
end $$;

-- Step 2: defensively reclassify structurally-invalid legacy rows before
-- enforcing the new constraints (V2's superseded draft table shape).
...
-- Step 3: rename last_attempted_at -> lease_expires_at semantics (C).
-- The prior draft used last_attempted_at as a heartbeat; the new design
-- uses a fixed lease set once at creation. Existing 'reserved' rows from
-- prior test fixtures get a fresh 5-minute lease from now (they are
-- synthetic test fixtures, not real in-flight scans, so backdating is not
-- meaningful -- they will be picked up by the reconciler on the next run
-- if still 'reserved' after 5 minutes, which is the correct behavior for
-- stale test fixtures).
alter table public.keyword_scan_reservations add column if not exists lease_expires_at timestamptz;

update public.keyword_scan_reservations
  set lease_expires_at = case when status = 'reserved' then now() + interval '5 minutes' else null end
  where lease_expires_at is null;

alter table public.keyword_scan_reservations drop column if exists last_attempted_at;

-- Step 4: constraints (drop-if-exists first, this table has had several
-- draft iterations on this project).
...
alter table public.keyword_scan_reservations drop constraint if exists keyword_scan_reservations_reserved_has_lease_check;
alter table public.keyword_scan_reservations add constraint keyword_scan_reservations_reserved_has_lease_check check (
...
-- These three were baked into 01_production_migration.sql's CREATE TABLE
-- (fresh table there) but were missing from this ALTER-based reconciliation
-- (existing table here) -- found via structural comparison against 01's
-- resulting schema, not by inspection alone. Added to match exactly.
alter table public.keyword_scan_reservations drop constraint if exists keyword_scan_reservations_credit_source_check;
alter table public.keyword_scan_reservations add constraint keyword_scan_reservations_credit_source_check check (
  credit_source = any (array['free'::text, 'paid'::text])
);
alter table public.keyword_scan_reservations drop constraint if exists keyword_scan_reservations_idempotency_key_check;
alter table public.keyword_scan_reservations add constraint keyword_scan_reservations_idempotency_key_check check (
  length(idempotency_key) >= 8 and length(idempotency_key) <= 100
);
alter table public.keyword_scan_reservations drop constraint if exists keyword_scan_reservations_status_check;
alter table public.keyword_scan_reservations add constraint keyword_scan_reservations_status_check check (
  status = any (array['reserved'::text, 'completed'::text, 'released'::text])
);
...
-- Verify no pre-existing row violates the new constraints (the ADD
-- CONSTRAINT statements above already enforce this and would have failed
-- loudly, but re-confirmed explicitly).
do $$
...
begin
  ...
  if v_violations > 0 then raise exception 'unexpected: % rows violate result constraints', v_violations; end if;
end $$;

-- Step 5: remove old client SELECT policy, revoke direct table access.
...
-- Step 6: remove the unused test-only column.
alter table public.profiles drop column if exists keyword_scan_balance;

-- Step 7: drop superseded helper functions from earlier drafts.
drop function if exists public.restore_keyword_scan_credit(uuid, text, uuid);

-- Step 8: credit_batches -- new columns. keyword_scans_granted/remaining
-- were missing from an earlier draft of this reconciliation script (found
-- via a real runtime failure of reserve_keyword_scan against a reconciled
-- database during local validation, not by inspection alone) -- added here
-- to match 01_production_migration.sql Section B exactly.
...
-- Existing synthetic 'fictional_test_pack_*' rows from earlier Part B
-- testing are source='manual_grant', not 'purchase' -- the purchase-only
-- expiry/verified-facts constraints below do NOT apply to them, and they
-- are left untouched.
do $$
begin
  ...
  if not exists (select 1 from pg_constraint where conrelid='public.credit_batches'::regclass and conname='credit_batches_purchase_verified_facts_check') then
    alter table public.credit_batches add constraint credit_batches_purchase_verified_facts_check check (
      source <> 'purchase' or (stripe_price_id is not null and amount_paid is not null and currency is not null and quantity is not null and paid_at is not null)
    );
  end if;
end $$;

create unique index if not exists credit_batches_stripe_session_unique_idx
  on public.credit_batches (stripe_checkout_session_id) where stripe_checkout_session_id is not null;
...
-- Step 9: check_ledger -- same extensions as production.
...
alter table public.check_ledger add column if not exists keyword_scan_reservation_id uuid references public.keyword_scan_reservations(id);

alter table public.check_ledger drop constraint if exists check_ledger_entry_type_check;
...
alter table public.check_ledger add constraint check_ledger_credit_type_check check (credit_type in ('check', 'keyword_scan'));

create unique index if not exists check_ledger_reservation_used_unique_idx on public.check_ledger (keyword_scan_reservation_id) where entry_type = 'used' and keyword_scan_reservation_id is not null;
create unique index if not exists check_ledger_reservation_released_unique_idx on public.check_ledger (keyword_scan_reservation_id) where entry_type = 'released' and keyword_scan_reservation_id is not null;
create unique index if not exists check_ledger_batch_purchased_unique_idx on public.check_ledger (batch_id, credit_type) where entry_type = 'purchased';
create unique index if not exists check_ledger_batch_expired_unique_idx on public.check_ledger (batch_id, credit_type) where entry_type = 'expired';
create unique index if not exists check_ledger_batch_refunded_unique_idx on public.check_ledger (batch_id, credit_type) where entry_type = 'refunded';

-- Step 10: stripe_webhook_events -- test project has 0 rows (verified
-- earlier this session), so no backfill classification is needed; the
-- NOT NULL enforcement below applies to zero rows.
...
begin
  select count(*) into v_orphaned from public.stripe_webhook_events where event_type is null;
  if v_orphaned > 0 then raise exception 'stripe_webhook_events: % rows with null event_type on test project -- expected 0, stopping', v_orphaned; end if;
end $$;
alter table public.stripe_webhook_events alter column event_type set not null;

-- Step 11: feature_flags, canary allowlist, refund_events -- new on this project too.
...
insert into public.feature_flags (key, enabled) values ('keyword_scan_maintenance', true) on conflict (key) do nothing;

create table if not exists public.keyword_scan_canary_users (user_id uuid primary key references public.profiles(id));
...
create unique index if not exists refund_events_one_active_pending_idx on public.refund_events (batch_id) where status = 'pending';
create index if not exists refund_events_batch_idx on public.refund_events (batch_id, created_at desc);

-- Step 12: replace ALL RPC bodies with the exact literal statements
-- (function bodies, grants, cron schedules) below -- byte-identical to
-- 01_production_migration.sql Sections G-K, applied here in full.
...
do $$
begin
  raise notice 'Reconciliation complete. keyword_scan_reservations: %, credit_batches: %, constraints on reservations: %',
