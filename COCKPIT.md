...
## 2026-09-23 — Free Keyword Scan renamed to Free ATS Check in user-facing copy (PR #167)

**Objective.** Founder wanted the free, pre-purchase "Free Keyword Scan"
feature relabelled "Free ATS Check", while confirming the product is still
not an ATS checker: it is a free, non-main-product feature. Discussed in
chat rather than sourced from a Notion decision; confirmed relevant because
the site already defines "an ATS check" elsewhere as keyword matching only
(`RecruiterEvaluationPage.tsx`, `AtsResumeCheckerPage.tsx` FAQ copy), which
is exactly what this free feature does.

**Completed.** User-facing copy only, in
[`src/pages/KeywordScanPage.tsx`](src/pages/KeywordScanPage.tsx) (meta
title/description, page heading, submit button and its loading state),
[`src/pages/NewCheckPage.tsx`](src/pages/NewCheckPage.tsx) and
[`src/pages/MyChecksPage.tsx`](src/pages/MyChecksPage.tsx) (CTA labels),
[`src/pages/FaqPage.tsx`](src/pages/FaqPage.tsx) (the free-scan FAQ entry),
and [`public/llms-full.txt`](public/llms-full.txt). Deliberately untouched:
the `/checks/keyword-scan` route, the `keyword-scan` edge function, DB
columns (`keyword_scan_reservations`, `keyword_scans_granted`,
`keyword_scans_consumed`, `free_keyword_scans_available`), `runKeywordScan`,
`FREE_SCAN_LIMIT`, analytics event names, code comments, and the separate
`/cv-keyword-checker` and `/ats-resume-checker` SEO landing pages (a
distinct, larger decision, out of scope here per discussion).
`scripts/csp-managed-hashes.json` and `vercel.json` were regenerated by
`npm run build`, since the FAQ page's structured data content changed
(one hash swapped for the new Q&A text).

**Verified.** `npm run lint`, `npm run typecheck`, `npm run test:unit`
(24/24 files, 284 assertions) and `npm run build` (prerender + sitemap) all
passed. Confirmed the rebuilt `dist/faq/index.html` and `dist/llms-full.txt`
contain the new copy, and manually loaded `/faq` on `localhost:5173`
through the Chrome connector to confirm the rendered Q&A text.

**Blockers.** None technical.

**Founder action required.** Review and merge PR #167. The authenticated
pages this touches (`/checks/keyword-scan`, `/checks/new`, `/checks`)
could not be visually confirmed locally, since local dev has no Supabase
session configured and redirects unauthenticated; worth a quick look with a
logged-in account after merge, before or after it ships. Merging deploys
the frontend through Vercel's GitHub integration; no Edge Function is
touched, so nothing deploys from the Actions workflow.

**Next technical step.** None planned. If the founder later wants
`/ats-resume-checker`'s CTA reconciled with this free feature, or the
`/cv-keyword-checker` SEO page's naming reconsidered, that is a separate
piece of work raised but not scoped here.
