...
export const SAMPLE_WORDING_CALIBRATION_EXAMPLES = [
  ...
  'Collaborated with 3 engineering and data science teams to define requirements for a new recommendation feature, contributing to the launch of 2 production releases.',
] as const
...
Every sample wording field is one complete, realistic CV bullet the candidate could adapt using their real experience and results. The product labels it "Sample wording" and tells the candidate it is a fictional example, so it must SHOW what strong CV evidence looks like, never TELL the candidate what to do. The quantification freedom described in STEP 1 and STEP 2B above is about how you score and describe the candidate's real CV; it never applies here, sample wording always follows rule 5 below. All of these rules are mandatory:
...
5. Include at least one measurable number: a count, a duration, a percentage, or a scale. Whenever the bullet involves other people, such as a team, stakeholders, customers, or an audience, that is always a natural count, so quantify how many people, teams, or sessions were involved, exactly like a technical bullet, never wave a collaboration, stakeholder, or communication bullet through as "soft" or uncountable. Wherever the bullet also names a concrete result, such as an adoption, a shipped deliverable, or a follow on request, quantify that outcome too, so a bullet like this uses two numbers, one for scope and one for outcome: "Collaborated with 3 engineering and data science teams to define requirements for a new recommendation feature, contributing to the launch of 2 production releases." Skip a number only for a bullet about a soft skill's own quality in the abstract, such as attention to detail or tone, where literally nothing in the sentence names a person, group, frequency, or result to count; the moment it names any of those, this exception no longer applies. Write every number as a digit: "5", not "five"; "2 days", not "two days".
...
- Cross team collaboration, technical or product role: "${SAMPLE_WORDING_CALIBRATION_EXAMPLES[4]}"
