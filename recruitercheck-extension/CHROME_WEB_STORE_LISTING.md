...
## Extension name
RecruiterCheck — Job Capture

## Summary (132 characters max)
Capture the job posting you're viewing in one click and send it to RecruiterCheck for a recruiter's-eye check.
...
RecruiterCheck — Job Capture does one thing: it gets the job posting you're
looking at into RecruiterCheck, without you copying and pasting a word.

HOW IT WORKS
1. Open a job posting on LinkedIn, Indeed, or almost any company careers page.
2. Click the RecruiterCheck icon and press "Capture this job."
3. Review the captured title, company, and description.
4. Press "Check this job" — it opens in RecruiterCheck with the job already
...
- Requires its own one-time connection to your RecruiterCheck account (no
...
A RecruiterCheck account is required (free tier available) — sign up at
...
only, and sends it to the user's own RecruiterCheck account to pre-fill a
new Recruiter Check.

## Permission justifications

**storage** — stores the extension's own RecruiterCheck session (separate
...
**identity** — used only for the one-time "Connect RecruiterCheck" flow,
which opens a browser-controlled window for the user to authorize the
extension on their existing, already-signed-in RecruiterCheck account. No
