...
export function FileDropzone({
  ...
  if (fileName) {
    return (
      <div
        key={fileName}
        className="file-selected-row flex items-center gap-3 rounded-[10px] border border-white/25 bg-white/[0.06] px-3 py-2.5"
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          {busy ? <p className="text-xs text-white">{busyLabel}</p> : null}
        </div>
        <div className="flex shrink-0 items-center gap-1">
          ...
          {onRemove ? (
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-lg text-white transition-colors hover:bg-white/10 hover:text-error-light disabled:pointer-events-none disabled:opacity-50"
            >
              <TrashIcon />
            </button>
          ) : null}
    ...
    <div>
      <div
        ...
        className={cn(
          ...
          dragActive ? 'border-blue-light bg-blue-light/10' : 'border-white/30 bg-white/[0.05] hover:border-blue-light/50',
        )}
      >
      ...
      <div id={`${id}-helper`} className="mt-1.5 space-y-1 text-xs text-white">
        {helperText}
      </div>
      {hiddenInput}
    </div>
